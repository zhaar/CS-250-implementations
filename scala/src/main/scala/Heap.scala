...
object Heap {
  ...
  * This implentation is mostly borrowed and adapted from the book
  * "Purely functional data structures" by Okasaki
  *
  * Leftist heaps are not balanced but merging is pretty cheap and
  * the minimum is also at the root
  */
  sealed trait Heap[+T] {
    def rank(): Int
  ...
  def makeT[T](elem: T, a: Heap[T], b: Heap[T]): Heap[T] =
    ...
    case object EmptyHeap extends Heap[Nothing] {
      ...
      def insert[S](elem: S)(implicit ord: Ordering[S]): Heap[S] = {
        println("inserting " + elem + " into empty Heap")
        makeNode(elem)
      }
    ...
    case class HeapNode[T](r: Int, e: T, left: Heap[T], right: Heap[T]) extends Heap[T] {
      ...
      def insert[S >: T](elem: S)(implicit ord: Ordering[S]): Heap[S] = {
        ...
        this.merge[S](makeNode(elem))(ord)
      }
    ...
    def heapAsList[T](heap: Heap[T]): List[T] = {
      def loop(ls: List[T], h: Heap[T]): List[T] = heap match {
        case EmptyHeap => ls
        case HeapNode(_, elem, lft, rgt) => elem :: heapAsList(lft) ::: heapAsList(rgt) ::: ls
      }
      loop(Nil, heap)
    }
    def buildMaxHeap[T](list: List[T])(implicit ord: Ordering[T]): Heap[T] =
      list.foldLeft(EmptyHeap: Heap[T])((heap, elem) => {
